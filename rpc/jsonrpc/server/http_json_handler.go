	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			if wErr := WriteRPCResponseHTTPError(w, http.StatusBadRequest, res); wErr != nil {
				logger.Error("failed to write response in makeJSONRPCHandler StatusBadRequest", "err", wErr)
			}
			return
		}
		...
		if err := json.Unmarshal(b, &requests); err != nil {
			...
			if err := json.Unmarshal(b, &request); err != nil {
				...
				if wErr := WriteRPCResponseHTTPError(w, http.StatusInternalServerError, res); wErr != nil {
					logger.Error("failed to write response makeJSONRPCHandler StatusInternalServerError", "err", wErr)
				}
				return
			}
		...
		if len(responses) > 0 {
			...
			if wErr != nil {
				logger.Error("failed to write responses end of makeJSONRPCHandler", "err", wErr)
			}
		}
	}
}
