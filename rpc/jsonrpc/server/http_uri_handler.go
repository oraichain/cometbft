	...
	if rpcFunc.ws {
		return func(w http.ResponseWriter, r *http.Request) {
			...
			if wErr := WriteRPCResponseHTTPError(w, http.StatusNotFound, res); wErr != nil {
				logger.Error("failed to write response makeHTTPHandler StatusNotFound", "err", wErr)
			}
		}
	}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			if wErr := WriteRPCResponseHTTPError(w, http.StatusInternalServerError, res); wErr != nil {
				logger.Error("failed to write response makeHTTPHandler StatusInternalServerError", "err", wErr)
			}
			return
		}
		...
		if err != nil {
			if err := WriteRPCResponseHTTPError(w, http.StatusInternalServerError,
				...
				logger.Error("failed to write response makeHTTPHandler StatusInternalServerError RPCInternalError", "err", err)
				return
			}
			return
		}
		...
		if err != nil {
			logger.Error("failed to write response makeHTTPHandler WriteRPCResponseHTTP", "err", err)
			return
		}
	}
}
